                             ...
                             celltypeProp_cutoff2show = 0.0)
{
  ...
  #for(n in c(1, 2, 4))
  {
    ...
    if(PLOT.scatterpie){
      ...
      ggsave(paste0(resultsdir, '/RCTD_scatterpie_', slice, '.pdf'), width = 16, height = 10)
      ...
      for(m in 1:length(cell_types_plt)){
        ...
        Add_customizedCellProprotion_plot = TRUE
        if(Add_customizedCellProprotion_plot){
          ...
          p0 = SpatialPlot(stx_tmp, images =  slice, features = "celltype", 
                      slot = 'data',
                      min.cutoff = celltypeProp_cutoff2show,
                      image.alpha = 0.8
                      ) +
            ggtitle(cell_types_plt[m])
          plot(p0)
          
          image <- slice %||% Images(object = stx_tmp, assay = DefaultAssay(object = stx_tmp))
          features = "celltype"
          
          data <- FetchData(object = stx_tmp, vars = features, slot = 'data')
          features <- colnames(x = data)
          
          colnames(x = data) <- features
          rownames(x = data) <- Cells(x = stx_tmp)
          
          features <- colnames(x = data)
          colnames(x = data) <- features
          rownames(x = data) <- colnames(x = stx_tmp)
          
          image.use <- stx_tmp[[image[[1]]]]
          coordinates <- GetTissueCoordinates(object = image.use)
          
          data = cbind(coordinates, data[rownames(x = coordinates), features, drop = FALSE])
          
          geom = "spatial"; pt.alpha = NULL;
          
          data_reverse = data
          data_reverse$imagerow = max(data_reverse$imagerow) - data_reverse$imagerow + 
            min(data_reverse$imagerow)
          
          p2 = ggplot(data = data_reverse, aes(x = imagecol, y = imagerow)) +
            geom_point(aes(size = celltype), color = col_ct[m]) +
            scale_size(limits = c(celltypeProp_cutoff2show, 1), 
                       breaks = c(celltypeProp_cutoff2show, 0.2,0.4, 0.6, 0.8, 1)) 
            
          p2 <- p2 + image_background(data = data, 
                                          image = image.use, 
                                          point.size.factor = 0., 
                                          image.alpha = 0.4,
                                          #image.scale = "lowres",
            ...
            NoAxes() + theme(panel.background = element_blank())
...
Radius_VisiumV1 <- function(object, scale = "lowres", ...) {
...
#'
GeomSpatial_customized <- ggproto(
  "GeomSpatial_customized",
  Geom,
  ...
  draw_panel = function(data, panel_scales, coord, image, image.alpha, image.scale, crop) 
  {
    ...
    spot.size <- Radius_VisiumV1(object = image, scale = image.scale)
