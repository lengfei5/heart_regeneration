...
if(Run_RCTD_axolotl_allVisium){
  ...
  #RCTD_out = paste0(resDir, '/RCTD_allVisium_subtype_out_41subtypes_ref.time.specific_v3.0')
  #RCTD_out = paste0(resDir, '/RCTD_out', 
  #                  '/RCTD_subtype_out_41subtypes_ref.time.specific_v3.5_ventricleRegion/')
  #system(paste0('mkdir -p ', RCTD_out))
  
  st = readRDS(file = paste0(RCTD_out, '/RCTD_st_axolotl_allVisium.rds'))
  refs = readRDS(file = paste0(RCTD_out, '/RCTD_refs_subtypes_final_20221117.rds'))
  condition.specific_celltypes = readRDS(
