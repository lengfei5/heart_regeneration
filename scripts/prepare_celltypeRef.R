...
saveRDS(aa, file = paste0(RdataDir, 'Seurat.obj_adultMiceHeart_week0.week2_Ren2020.rds'))

####### KEEP the umap configuration for only week0: nfeatures = 2000, dims = 1:20, n.neighbors = 30 and min.dist = 0.05 
####### umap configuraiton for week0 and week2: nfeatures = 3000, dims = 1:20, n.neighbors = 30/50, min.dist = 0.1
...
(p1 + p2)/(p3 + p4) + ggsave(paste0(resDir, '/Umap_Ren2020_week0.week2_newClusters_vs_cellType.original_conditions_samples.pdf'), 
                             width = 12, height = 8)
...
p2 / p5 + ggsave(paste0(resDir, '/Umap_cellType.original_FeatuerPlot_cardiomyocyoteMarkers.pdf'), 
                  width = 14, height = 20)

p6 = VlnPlot(aa, features = gg.examples, group.by = 'CellType')

p6 + ggsave(paste0(resDir, '/Umap_VlnPlot_cardiomyocyoteMarkers.pdf'), 
            width = 14, height = 10)
